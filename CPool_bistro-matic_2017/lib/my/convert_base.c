...
char *convert_calc(char *base, char *sign, char *expr)
{
	int i = 0;
	int i_nexpr = 0;
	char *new_expr = malloc(sizeof(char) * strlen(expr));
	char *get_base = malloc(sizeof(char) *
		(strlen(sign) + strlen(base)));

	get_base = get_new_base(sign, base);
	while (expr[i] != '\0') {
		new_expr[i_nexpr] = conv_norm_base(expr[i], get_base);
		i_nexpr = i_nexpr + 1;
		i = i + 1;
	}
	my_putstr(new_expr);
